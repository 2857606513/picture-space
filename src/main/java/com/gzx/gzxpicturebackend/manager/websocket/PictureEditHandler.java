package com.gzx.gzxpicturebackend.manager.websocket;
...
import com.gzx.gzxpicturebackend.manager.websocket.disruptor.PictureEditEventProducer;
import com.gzx.gzxpicturebackend.manager.websocket.model.PictureEditActionEnum;
import com.gzx.gzxpicturebackend.manager.websocket.model.PictureEditMessageTypeEnum;
import com.gzx.gzxpicturebackend.manager.websocket.model.PictureEditRequestMessage;
import com.gzx.gzxpicturebackend.manager.websocket.model.PictureEditResponseMessage;
...
public class PictureEditHandler extends TextWebSocketHandler {
    ...
    // TODO: 1、为防止消息丢失，可以使用 Redis 等高性能存储保存执行的操作记录。目前如果图片已经被编辑了，新用户加入编辑时没办法查看到已编辑的状态，这一点也可以利用 Redis 保存操作记录来解决，新用户加入编辑时读取 Redis 的操作记录即可。2、支持分布式 WebSocket。实现思路很简单，只需要保证要编辑同一图片的用户连接的是相同的服务器即可，和游戏分服务器大区、聊天室分房间是类似的原理。3、一些小问题的优化：比如 WebSocket 连接建立之后，如果用户退出了登录，这时 WebSocket 的连接是没有断开的。不过影响并不大，大家可以思考下怎么处理。
}

package com.gzx.gzxpicturebackend.api.aliyunai;
...
public class AliYunAiApi {
     ...
     * @return
     */
    public AiOutPaintingResponse aiOutPaintingTask(AiOutPaintingRequest aiOutPaintingRequest) {

        ThrowUtils.throwIf(aiOutPaintingRequest == null,ErrorCode.OPERATION_ERROR);
        // 发送请求
        HttpRequest httpRequest = HttpRequest.post(CREATE_OUT_PAINTING_TASK_URL)
                ...
                .body(JSONUtil.toJsonStr(aiOutPaintingRequest));
        // 处理响应
        try (HttpResponse httpResponse = httpRequest.execute()) {
            ...
            AiOutPaintingResponse aiOutPaintingResponse = JSONUtil.toBean(httpResponse.body(), AiOutPaintingResponse.class);
            if (aiOutPaintingResponse.getCode() != null) {
                String errorMessage = aiOutPaintingResponse.getMessage();
                log.error("请求异常：{}", errorMessage);
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "AI 扩图失败，" + errorMessage);
            }
            return aiOutPaintingResponse;
        }
    }
    ...
    public GetOutPaintingTaskResponse getOutPaintingTask(String taskId) {
        ThrowUtils.throwIf(StrUtil.isBlank(taskId),ErrorCode.OPERATION_ERROR, "任务 ID 不能为空");
        // 处理响应
